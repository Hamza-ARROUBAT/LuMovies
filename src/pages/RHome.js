...
export default function Home() {
  ...
  const [moviesNumber, setMoviesNumber] = useState(movies.length);
  ...
  const sliceData = (dataArray, page, arrayLimit, choosenCategory) => {
    if (choosenCategory) {
      ...
      setMoviesNumber(filtred.length);
      setPages(
      ...
      setMoviesNumber(dataArray.length);
      setPages(
  ...
  const handleDelete = (id) => {
    ...
    if (category) {
      ...
      newFiltredMoviesArray = filtredMovies.filter((movie) => movie.id !== id);
      setFiltredMovies(newFiltredMoviesArray);
      setMoviesNumber(moviesNumber - 1);
      setPages(
        Array(Math.ceil(moviesNumber / limit))
          .fill(0)
          .map((element, index) => index)
      );
